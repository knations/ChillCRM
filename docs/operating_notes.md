...
Use `POST https://chillcrm.app/api/webhooks/zapier_purchase` for Zapier shopping-cart purchase intake after setting the server-side `CHILLCRM_ZAPIER_WEBHOOK_SECRET` environment variable in Vercel. Zapier should send `Content-Type: application/json` and either `Authorization: Bearer <secret>` or `X-CHILLCRM-WEBHOOK-SECRET: <secret>`. The payload must include a customer email and may include `name`, `first_name`, `last_name`, `phone`, `mobile`, `order_id`, `transaction_id`, `product_name`, `total`, `currency`, `purchased_at`, and common address fields such as `address.line1`, `address.city`, `billing.address1`, `billing.zip`, `shipping.address1`, or flat `address1`, `city`, `state`, `zip`, `country`. ThriveCart-through-Zapier aliases are also accepted, including `customer_name`, `customer_email`, `customer_phone`, `customer_first_name`, `customer_last_name`, `product_label`, `purchase_amount`, `purchase_currency`, `order_date`, `customer_address`, `customer_city`, `customer_state`, `customer_zip`, and `customer_country`. CHILLCRM finds an existing Person by email, fills only blank contact basics, fills the primary address only when it is blank, appends the purchase as a local note containing Date, Product Name, Address, and Price, skips duplicate order/transaction deliveries, and creates a new Person only when the email is not already present.

Use `scripts/set_vercel_zapier_webhook_secret.py` for the non-secret-safe Vercel setup step that generates or accepts a `CHILLCRM_ZAPIER_WEBHOOK_SECRET`, upserts it for production, preview, and development, and writes only proof metadata to `reports/vercel_zapier_webhook_secret_status.md`. The desktop launcher `ops/operator_launchers/Run Vercel Zapier Webhook Setup.command` wraps that script with confirmation, hidden Vercel token entry, and follow-up Vercel environment/deployment report refreshes. Neither path deploys code, unlocks writes, stores secret values, switches source of truth, or changes CRM records.
