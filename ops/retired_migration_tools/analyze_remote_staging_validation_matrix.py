#!/usr/bin/env python3
"""Generate a read-only remote staging validation matrix."""
...
def generate_report(rows: list[dict[str, Any]]) -> str:
    ...
    expected_counts = rows_of_type(rows, "expected_count")
    sections = rows_of_type(rows, "validation_section")
    checks = rows_of_type(rows, "validation_check")
    blocker_rules = rows_of_type(rows, "blocker_rule")
    next_steps = rows_of_type(rows, "next_step")

    lines = [
        "# Remote Staging Validation Matrix",
        "",
        f"Generated: {generated_at}",
        "",
        "This is a read-only validation matrix for the hosted Supabase/Vercel staging CRM. It defines the checks, expected local counts, evidence fields, blocker rules, owner-shakedown gates, optional internal-user gates, and cutover gates. It does not unlock writes, create accounts, invite internal users, save decisions, expose localhost, switch source of truth, or change CRM records.",
        "",
        "## Summary",
        "",
        ...
        *table(
        ...
        "## Safety Boundary",
        "",
        "This matrix is tracking only. It does not unlock hosted writes, create users, invite internal users, save decisions, expose this machine, switch source of truth, or change local CRM records. Actual pass/fail evidence is filled only after each hosted proof gate is verified.",
        "",
        "## Related Files",
        "",
...
def main() -> int:
    ...
    app = handler()
    rows = app.export_remote_staging_validation_matrix_rows()
    write_csv(REPORTS_DIR / "remote_staging_validation_matrix.csv", rows)
    (REPORTS_DIR / "remote_staging_validation_matrix.md").write_text(generate_report(rows), encoding="utf-8")
    print(f"Wrote {len(rows):,} remote staging validation rows to reports/remote_staging_validation_matrix.md and .csv")
    return 0
