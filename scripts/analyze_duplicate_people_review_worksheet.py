#!/usr/bin/env python3
"""Generate a read-only duplicate people review worksheet."""
...
def generate_report(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "# Duplicate People Review Worksheet",
        "",
        f"Generated: {generated_at}",
        "",
        "This is a read-only working packet for duplicate-person cleanup review. It does not save the project policy, save group decisions, merge records, delete records, resolve cleanup flags, or update legacy CRM provider.",
        "",
        "## Summary",
        "",
        ...
        f"- Open duplicate-person groups: {int(summary.get('open_groups') or 0):,}.",
        f"- Review remaining: {int(summary.get('review_remaining') or 0):,}.",
        f"- High-priority groups: {int(summary.get('high_priority_groups') or 0):,}.",
        f"- People records involved: {int(summary.get('records_involved') or 0):,}.",
        ...
        *table(
        ...
        "## Safety Boundary",
        "",
        "- The worksheet is evidence only; it does not save the duplicate people policy.",
        ...
        "- Future merge execution should remain disabled until a fresh backup, exact affected counts, undo path, and final confirmation exist.",
        "",
        "## Related Files",
        "",
...
def main() -> int:
    ...
    app = handler()
    rows = app.export_duplicate_people_review_worksheet_rows()
    write_csv(REPORTS_DIR / "duplicate_people_review_worksheet.csv", rows)
    (REPORTS_DIR / "duplicate_people_review_worksheet.md").write_text(generate_report(rows), encoding="utf-8")
    print(f"Wrote {len(rows):,} duplicate people worksheet rows to reports/duplicate_people_review_worksheet.md and .csv")
    return 0
