...
def generate_report(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "This is a read-only staging data load plan for moving the CHILLCRM into a hosted database after a hosting posture is chosen. It does not create a remote database, provision hosting, migrate data, create users, save decisions, invite admins, or change CRM records.",
        "",
        "## Summary",
        "",
        ...
        *table(
        ...
        "## Recommendation",
        "",
        "Use this plan after the hosted schema draft is reviewed. Load a staging copy from the local SQLite database, preserve local IDs, upload private files, seed only staging users/roles, verify counts and permissions, and keep the CHILLCRM as the source of truth until the production cutover gate is explicitly approved.",
        "",
        "## Safety Boundary",
        "",
        "This load plan is planning only. It does not connect to a remote host, create a database, upload files, create users, save decisions, or change CHILLCRM records.",
        "",
        "## Related Files",
        "",
