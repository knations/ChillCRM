...
def generate_report(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "This is a read-only build blueprint for turning the CHILLCRM into a secure hosted CRM for you and admins. It does not provision hosting, create users, migrate data, expose localhost, save decisions, invite admins, or change CRM records.",
        "",
        "## Summary",
        "",
        ...
        *table(
        ...
        "## Recommendation",
        "",
        "Treat the CHILLCRM as the live source of truth while building this in staging. Build the hosted database and remote-only identity/file/audit tables first, then add role checks and actor-aware audit, run a staging pilot, and only then freeze local edits for production cutover.",
        "",
        "## Safety Boundary",
        "",
