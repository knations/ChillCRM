...
def generate_report(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        *table(
        ...
        "## Recommendation",
        "",
        "Use this as the active project board for the selected Supabase/Vercel path. Keep the CHILLCRM as the source of truth until provider backup/restore evidence, hosted write-audit rehearsal, monitoring readiness, and owner-only staging shakedown pass; optional internal admin access can follow only after owner signoff.",
        "",
        "## Safety Boundary",
        "",
        "This board is tracking only. It does not unlock hosted writes, create users, invite internal users, save decisions, expose this machine, switch source of truth, or change CHILLCRM records.",
        "",
        "## Related Files",
        "",
